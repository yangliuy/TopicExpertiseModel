...
/**Topic Sensitive PageRank Algorithm (CIKM 12 TSPR for Expert finding)
 * @author yangliu
...
public class TSPR {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		// load model
		//String modelFile = PathConfig.modelOutPath + "/USER80/ModelFile.model";
		...
		//System.out.println(model.theta.length);
		...
		double [][] theta = FileUtil.read2DArray(LDAThetaFile);
	...
	private static void computeTransM(double[][] theta, int z) {
		// TODO Auto-generated method stub
		double[][] transm = new double[NODENUM][NODENUM];
		for(int i = 0; i < NODENUM; i++){
			double rowSum = 0;
			for(int j = 0; j < NODENUM; j++){
				rowSum += graphAdjM.get(i, j) * sim(theta[i][z], theta[j][z]);
			}
			if(rowSum == 0){
				for(int j = 0; j < NODENUM; j++){
					transm [i][j] = 0;
				...
				for(int j = 0; j < NODENUM; j++){
					double norWeight = graphAdjM.get(i, j) * sim(theta[i][z], theta[j][z]) /
										rowSum;
	...
	private static double sim(double f, double g) {
		// TODO Auto-generated method stub
	 ...
	 * @return NEWPR matrix
	 */
	public static Matrix computeNewPR(double[][] theta, int z) {
		...
		for(int i = 0; i < NODENUM; i++){
			double userPreference = theta[i][z];
