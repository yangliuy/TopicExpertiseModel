...
public final class MatrixOps
{
	...
	public static void arrayListToArray(ArrayList<Integer> arrList, int[] arr){
		assert( arrList.size() == arr.length);
		for(int i = 0; i < arrList.size(); i++){
			arr[i] = arrList.get(i);
		}
	}
	
	public static void arrayListToArray(ArrayList<Double> arrList, double[] arr){
		assert( arrList.size() == arr.length);
		for(int i = 0; i < arrList.size(); i++){
			arr[i] = arrList.get(i);
		}
    ...
    public static void set (int[] dest, int[] source) {
		// XXX  Huh? This if-statement won't work. Java is pass-by-value, buddy. -cas
	    if (source.length != dest.length)
  ...
  public static double[] append (double[] original, double newValue)
  {
	...
	public static double dot(double[] m1, short[] m2) {
		 double ret = 0.0;
	     for (int i=0; i < m1.length; i++)
	        ret += m1[i] * (double)m2[i];
	     return ret;
	}
}
